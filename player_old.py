...
class Player:
    ...
    def animate(self):
        if self.alive:
            # BODY
            if not self.abilities[Ability.gun] or self.laddered:
                if self.grounded:
                    if self.crouched:
                ...
                if self.grounded:
                    if self.crouched:
                        if self.cooldown > 0:
                            self.sprite_body.play('gun_crouch_attack')
                        else:
                            self.sprite_body.play('gun_crouch')
                    elif self.climbing:
                        self.sprite_body.play('gun_up')
                    else:
                        if self.cooldown > 0:
                            self.sprite_body.play('gun_attack')
                        else:
                            if self.dx == 0:
                                self.sprite_body.play('gun_idle')
                            else:
                                self.sprite_body.play('gun_walk')
                else:
                    if self.cooldown > 0:
                        self.sprite_body.play('gun_attack')
                    else:
                        if self.dy < 0:
                            self.sprite_body.play_once('gun_jump', 0)
                        elif self.dy > 0:
                            self.sprite_body.play_once('gun_jump', 2)
                        else:
                            self.sprite_body.play_once('gun_jump', 1)
    ...
    def apply_powerups(self, room):
        ...
        for p in pygame.sprite.spritecollide(self, room.powerups, False):
            if self.abilities[p.ability] is False:
                ...
                if p.ability is Ability.full_auto or p.ability is Ability.spread or p.ability is Ability.gravity:
